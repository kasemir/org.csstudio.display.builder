...
public class PropertyPanel extends BorderPane {
    ...
    private void filterProperties ( String search ) {

        List<Widget> selection = editor.getWidgetSelectionHandler().getSelection();
    ...
    private void setSelectedWidgets ( final List<Widget> widgets ) {
        ...
        if ( widgets.isEmpty() ) {   // Use the DisplayModel
            if ( model != null ) {
                updatePropertiesView(model.getProperties(), Collections.emptyList());
            }
        } else {   // Determine common properties
